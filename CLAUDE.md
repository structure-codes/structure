...
- **State + React stack:** state is **Jotai** (migrated off Recoil to unblock newer React). The app is on **React 18** (`createRoot` in [main.tsx](src/app/main.tsx)). Data fetching is **`@tanstack/react-query` v5** (migrated off the unmaintained `react-query` v3). Clipboard uses the native `navigator.clipboard` (the old `react-copy-to-clipboard` dep was dropped). A batch of unused deps (`react-d3-tree`, `react-flow-renderer`, `react-organizational-chart`, `react-syntax-highlighter`, `prismjs`) was removed. **No third-party dep blocks React 19 anymore** — bumping to it is just `react`/`react-dom`/`@types/*`→19 plus `@testing-library/react`→16 (the v14 we use targets React 18; v16 adds a `@testing-library/dom` peer).
...
- React 18, `react-jsx` runtime, TS `strict`, `noEmit` (Vite/esbuild does the transpile; `tsc` is type-check only).

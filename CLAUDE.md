...
- **State + React stack:** state is **Jotai** (migrated off Recoil to unblock newer React). The app is on **React 18** (`createRoot` in [main.tsx](src/app/main.tsx)). Clipboard uses the native `navigator.clipboard` (the old `react-copy-to-clipboard` dep was dropped). A batch of unused deps (`react-d3-tree`, `react-flow-renderer`, `react-organizational-chart`, `react-syntax-highlighter`, `prismjs`) was removed. React 19 is now gated only on a `react-query` v3 → `@tanstack/react-query` v5 migration (the remaining dep with a React-17-only peer).

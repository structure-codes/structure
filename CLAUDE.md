...
npm run test:coverage   # vitest run --coverage (v8; text + html report in ./coverage)
...
The pure modules are unit-tested directly (no DOM): the layout pipeline ([layout.ts](src/app/HomeView/ModelPanel/layout.ts)), `depthColor`, `oklch`, and `treeHelper` each have a `__tests__` sibling. Notably, `layout.test.ts` includes a **matrix invariant test** asserting `codeLineIds` emits exactly the lines `treeJsonToString` renders across every depth/hideFiles/hideDots combination — this is the regression guard for the two-pane visibility invariant described below. (The token guardrail [check-tokens.mjs](scripts/check-tokens.mjs) skips `__tests__` dirs, since color tests legitimately assert on hex/oklch literals.) The React components and `hooks.ts` are not yet covered.
...
[CodePanel](src/app/HomeView/CodePanel/index.tsx) registers a custom `"tree"` Monaco language ([customLang.ts](src/app/HomeView/CodePanel/customLang.ts)) and theme, and a folding-range provider derived from the tree. The editor is **read-only by design** — this is not a stepping stone to an editable text pane. The pane exists to *render* `treeJsonToString(treeState)` with syntax highlighting and folding (and to reflect atom-driven hover/select decorations), serving as an in-app showcase of the `"tree"` language that the separate [vscode-tree-language extension](https://github.com/structure-codes/vscode-tree-language) provides. The graph (ModelPanel) and the source dropdown are the editing surfaces; the text pane is a viewer. There is a large `handleEditorChange` function for live tree-prefix auto-formatting that is **intentionally disabled** (the `onDidChangeModelContent` wiring is commented out) — it's an experiment kept for reference, not dead code and not a planned feature; leave it unless explicitly asked to build edit support. The branch-glyph constants/helpers live in [treeHelper.ts](src/app/HomeView/CodePanel/treeHelper.ts) (`│`, `├──`, `└──`).

**Monaco bundling gotcha:** the actual editor is loaded from a CDN at runtime by `@monaco-editor/react`'s loader (there's no `loader.config({ monaco })` call), so monaco must **not** also be bundled. Both CodePanel files therefore import the npm package **types-only** (`import type * as monaco from "monaco-editor"`); the few runtime values needed (currently just `monaco.Range` for decorations) are taken from the live instance handed to `onMount` and stashed in `monacoRef`, not from a value import. Do **not** reintroduce `import * as monaco from "monaco-editor/esm/..."` — a value import re-bundles the whole editor (~1.5 MB) on top of the CDN copy. (Use the package-root specifier, not the deep `esm/...` path: monaco's `exports` map breaks deep-path type resolution under `moduleResolution: Bundler`.)
...
- **CI/deploy:** **Netlify owns deploys** via its GitHub Git integration — production on merge to `main`, a Deploy Preview per PR — both running the `[build]` command in [netlify.toml](netlify.toml) (`npm run build`, Node pinned to `.nvmrc`'s `20`). **GitHub Actions owns the PR quality gate**: [.github/workflows/ci.yaml](.github/workflows/ci.yaml) runs `npm ci → lint → build` on every PR to `main` (no tests in CI — the suite is fast, add `npm test` to that job if you want it gated too). So Netlify never lints/tests; keep the two concerns split rather than folding lint back into the Netlify build command.
- **Component styles: CSS Modules.** Each component has a sibling `style.module.css` imported as `import classes from "./style.module.css"` (default import named `classes`, so call sites stay `classes.foo`). All values come from the token CSS vars — no hardcoded colors/fonts/dimensions. The component library is **MUI v9 (`@mui/material`, emotion engine)**; there are no `makeStyles`/`useStyles` `style.ts` files. **Gotcha:** MUI's internal classes (`.MuiOutlinedInput-root`, `.MuiSlider-thumb`, …) are global, so wrap them in `:global(...)`. [main.tsx](src/app/main.tsx) wraps the app in `<StyledEngineProvider injectFirst>` so emotion injects its styles *first* and the CSS Modules win the cascade on order; as a belt-and-suspenders specificity margin, overrides applied to the *same element* as a MUI base class also double the local class (`.button.button`). Plain `.css` (non-module) is still used for the viz/Monaco internals ([tree.css](src/app/HomeView/ModelPanel/tree.css), [codePanel.css](src/app/HomeView/CodePanel/codePanel.css)) and the global utilities in [index.css](src/app/index.css).
...
- **Facelift status:** all phases complete. The MUI v4→v5 upgrade landed (`@mui/material` + `@emotion/*`, `palette.mode`, Autocomplete imported from core, `slotProps.input` replacing the removed `InputProps`, no `.MuiButton-label` wrapper) and the dependency has since been bumped to **v9**. MUI/emotion are **inlined** in [vite.config.mts](vite.config.mts) so jsdom can transform their native ESM (a leftover guard from when React 17 didn't expose `react/jsx-runtime`; harmless to keep).
- **State + React stack:** state is **Jotai** (migrated off Recoil). The app is on **React 19** (`createRoot` in [main.tsx](src/app/main.tsx)). Data fetching is **`@tanstack/react-query` v5** (migrated off the unmaintained `react-query` v3). Clipboard uses the native `navigator.clipboard` (the old `react-copy-to-clipboard` dep was dropped). Two batches of unused deps were removed: first `react-d3-tree`, `react-flow-renderer`, `react-organizational-chart`, `react-syntax-highlighter`, `prismjs`; then `lodash` (+`@types/lodash`), `nanoid`, and `clsx`. Tests use `@testing-library/react` v16 (with an explicit `@testing-library/dom` peer); coverage is via `@vitest/coverage-v8` (config in [vite.config.mts](vite.config.mts), report written to `./coverage`, gitignored).
- **Lint:** flat ESLint config. `@typescript-eslint/no-explicit-any` is **off** by design (API payloads + MUI handlers lean on `any`); unused vars are an error but `_`-prefixed names are ignored. The React Compiler hook ruleset is intentionally not enabled — only classic rules-of-hooks + exhaustive-deps (as a warning). `npm run lint` chains three checks (and CI runs all three): `eslint .`, the token guardrail, and `prettier --check .`.
- **Formatting:** Prettier owns formatting (config in [.prettierrc](.prettierrc); generated dirs and `*.md` are excluded via [.prettierignore](.prettierignore)). Run `npm run format` to fix, `npm run format:check` to verify; the latter is also part of `npm run lint`. ESLint carries no stylistic rules — they don't overlap.
